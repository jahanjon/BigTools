...
public static class ModelBuilderExtensions
{
    ...
    public static void AddPluralizingTableNameConvention(this ModelBuilder modelBuilder)
    {
        ...
        foreach (var entityType in modelBuilder.Model.GetEntityTypes())
        {
            if (!entityType.Name.Contains("View"))
            {
                var tableName = entityType.GetTableName();
    ...
    public static void AddSingularizingViewNameConvention(this ModelBuilder modelBuilder)
    {
        var pluralizer = new Pluralizer();
        foreach (var entityType in modelBuilder.Model.GetEntityTypes())
        {
            if (entityType.Name.Contains("View"))
            {
                ...
                entityType.SetViewName(pluralizer.Singularize(viewName));
            }
        }
    }
    ...
    public static void RegisterAllViewEntities<BaseType>(this ModelBuilder modelBuilder, params Assembly[] assemblies)
    {
        var types = assemblies.SelectMany(a => a.GetExportedTypes())
            .Where(c => c.IsClass && !c.IsAbstract && c.IsPublic && typeof(BaseType).IsAssignableFrom(c));
